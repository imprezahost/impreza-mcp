...
globalThis.fetch = async (url, options) => {
 ...
 count++;
 return new Response(JSON.stringify({success: true, data: {request_count: count, path: parsed.pathname, method: options.method, body: options.body ? JSON.parse(options.body) : null}}), {status: 200, headers: {'content-type': 'application/json'}});
};
